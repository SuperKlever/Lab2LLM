...
from src.graph.state import ConversationState, ProblemSolvingAgentAnswer
...
class ProblemSolvingAgent:
    ...
    def _create_chain(self, temperature: float):
        ...
        agent = create_agent(
            model=llm,
            tools=[calculator],
            response_format=ToolStrategy(ProblemSolvingAgentAnswer),
        )
        return PROBLEM_SOLVING_PROMPT | agent
